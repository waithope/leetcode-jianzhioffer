...
def convert(root):
    ...
    实现树到双向链表的转换。由于二叉搜索树的中序遍历就是按照从小到
    大的顺序进行访问，所以在访问的过程中，保存上一个节点的指针，也
    就是双向链表当前的尾节点指针，当访问到根节点的时候，将根节点的
    pLeft设置为上一个节点，如果上一个节点不为空，则把上一个节点的
    pRight设置为当前的根节点，这样一来，上一个节点和当前节点的双向
    就建立好了，一直递归下去，最后返回双向链表的尾节点，通过向前遍历
    到头节点，返回头节点。
    '''
