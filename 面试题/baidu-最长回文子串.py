...
def longestPalindrome_Manacher(s):
    '''
    思路：普通的最长回文子串算法存在两个问题，一个是长度奇偶性带来的对称轴位置问题
    以及重复访问的问题。manacher算法首先对字符串做预处理，在每个字符中间插入同样
    的一个符号（这个符号不在原串中出现），这样使得所得的串是奇数长度。
    假设处理后的字符串如下：
    S：# A # B # A # C # A # B #
    ...
    中的值，现在需要求第i个值。若此时i小于maxRight，说明以i为中心的回文串至少有一部分
